use crate::tui::app::{App, StreamDirectionFilter};
use crate::tui::collectors::packets::{CapturedPacket, ExpertSeverity, StreamDirection, parse_filter, matches_packet, port_label};
use ratatui::{
    prelude::*,
...
fn render_header(f: &mut Frame, app: &App, area: Rect, pkt_count: usize) {
    ...
    let mut line1_spans = vec![
        ...
        Span::raw("│ "),
        Span::raw("[1] Dashboard  [9] DDoS Logs  [2] Connections  [9] DDoS Logs  [3] Interfaces  [9] DDoS Logs  "),
        Span::styled("[4] Packets  [9] DDoS Logs", Style::default().fg(Color::Yellow).bold()),
        Span::raw("  [5] Stats  [9] DDoS Logs  [6] Topology  [9] DDoS Logs  [7] Timeline  [9] DDoS Logs  [8] Insights  [9] DDoS Logs"),
        Span::raw("  │ "),
        cap_status,
...
fn render_stream_view(f: &mut Frame, app: &App, area: Rect) {
    ...
    let proto_str = if stream.key.protocol == crate::tui::collectors::packets::StreamProtocol::Tcp {
        "TCP"
    } else {
        "UDP"
    };
