...
use crate::tui::collectors::connections::{Connection, ConnectionCollector, ConnectionTimeline};
use crate::tui::collectors::geo::GeoCache;
use crate::tui::collectors::health::HealthProber;
use crate::tui::collectors::insights::{InsightsCollector, NetworkSnapshot};
use crate::tui::collectors::packets::PacketCollector;
use crate::tui::collectors::traffic::TrafficCollector;
use crate::tui::collectors::whois::WhoisCache;
...
impl App {
    ...
    fn submit_insights_snapshot(&self) {
        ...
        let rx_rate = crate::tui::ui::widgets::format_bytes_rate(
            self.traffic.interfaces.iter().map(|i| i.rx_rate).sum(),
        );
        let tx_rate = crate::tui::ui::widgets::format_bytes_rate(
        ...
        match events.next().await? {
            AppEvent::Key(key) => {
                ...
                match key.code {
                    ...
                    KeyCode::Char('9') => app.current_tab = Tab::DdosLogs,
                    ...
                    KeyCode::Char('f') if app.current_tab == Tab::DdosLogs => {
                        app.ddos_logs_follow = !app.ddos_logs_follow;
                    }
                    KeyCode::Char('w') if app.current_tab == Tab::Packets => {
                        use crate::tui::collectors::packets::{
